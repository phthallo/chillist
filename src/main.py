...
from classes import multiline, checkClick, tooltip, dump, notify, resource_path
pygame.init()
pygame.mixer.init()
...
font = pygame.font.Font(resource_path("sysfont\sysfont\sysfont.ttf"), 15)
task_font = pygame.font.Font(resource_path("sysfont\sysfont\sysfont.ttf"), 20)
title_font = pygame.font.Font(resource_path("sysfont\sysfont\sysfont.ttf"), 25)
pygame.display.set_icon(pygame.image.load(resource_path(r"img\favicon.png")))
done = False              
scene = 0
...
if os.listdir(default_dir +"/mus") == []:
    for filename in os.listdir(resource_path("mus/")):
        file = os.path.join(resource_path("mus/"), filename)
...
while not done:
    ...
    if scene == 0: 
        if attributes["time_period"] == "afternoon" or attributes["time_period"] == "evening":
            backdrop = Backdrop(screen, resource_path("img/afternoon.png"))
            ...
            backdrop = Backdrop(screen, resource_path(f"img/{attributes['time_period']}.png"))
        ...
        backdrop = Backdrop(screen, resource_path("img/roomlinesSUNRISE.png"))
        ...
        if vinylWindow_open == True:
            ...
            control = Button(screen, resource_path("img/play.png"), x=400, y=190)
            if checkClick(control, pygame.mouse.get_pos(), event_list):
                ...
                if not pygame.mixer.music.get_busy() and firstPlay == False:
                    control = Button(screen,resource_path("img/play.png"), x=405, y=190)
                    currentlyplaying = random.choice(playlist)
                    pygame.mixer.music.load(currentlyplaying[0])
                    pygame.mixer.music.play()
                    clock.tick(10)
            ...
            if play == False:
                ...
                control = Button(screen,resource_path("img/pause.png"), x=405, y=190)
            ...
            matrix = Interactive(screen, "", img=resource_path("img/matrixmockup.png"), x=159, y=148, w=700, h=294)
            matrixlabel = Interactive(screen, "", img=resource_path("img/matrixlabel.png"),x=159, y=451)
            ...
            if placed == True:
                ...
                if event.type == pygame.KEYDOWN and event.key == pygame.K_RETURN:
                    ...
                    placed = False
            for i in stickers:
                tooltip(screen, resource_path("img/"+i["shape"]+".png"), task_font, i["desc"], x=i["coords"][0], y=i["coords"][1])
            ...
            todolayout = Interactive(screen, "", img=resource_path("img/timermockup.png"), x=130, y=130)
            control = Button(screen, resource_path("img/play_xs.png"), x=284, y=431)
            restart = Button(screen, resource_path("img/restart.png"), x=401, y=431)
            ...
            if timer_running == True:
                control = Button(screen, resource_path("img/pause_xs.png"), x=284, y=431)
            else:
                control = Button(screen, resource_path("img/play_xs.png"), x=284, y=431)
            if stickers:
                ...
                for sticker in stickers:
                    if stickers.index(sticker) <= 5:         
                        checkbox_rect = Interactive(screen, "", img=resource_path("img/checkboxes.png"), x=sticker["boxpos"][0], y=227+40*(stickers.index(sticker)))
                        if checkClick(checkbox_rect, pygame.mouse.get_pos(), event_list):
                            sticker["placed"] = True
                        if sticker["placed"] == True:
                            settings[0]["completed_tasks"] += 1
                            screen.blit(pygame.image.load(resource_path("img/cross.png")), pygame.image.load(resource_path("img/cross.png")).get_rect(topleft=(sticker["boxpos"][0], 227+40*(stickers.index(sticker)))))
